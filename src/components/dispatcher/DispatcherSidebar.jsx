...
const { FiHome, FiTruck, FiPackage, FiLogOut, FiMenu } = FiIcons;

const DispatcherSidebar = ({ isOpen, onToggle }) => {
  const location = useLocation();
  const { logout, user } = useApp();

  const menuItems = [
    { path: '/dispatcher', icon: FiHome, label: 'Dashboard', exact: true },
    { path: '/dispatcher/available', icon: FiPackage, label: 'Pedidos Disponibles' },
    { path: '/dispatcher/deliveries', icon: FiTruck, label: 'Mis Entregas' }
  ];
      ...
      <div className="p-4 border-b">
        <div className="flex items-center justify-between">
          <div className={`flex items-center ${isOpen ? '' : 'justify-center'}`}>
            ...
            {isOpen && (
              <div className="ml-3">
                <h1 className="text-lg font-bold text-gray-800">Despachador</h1>
